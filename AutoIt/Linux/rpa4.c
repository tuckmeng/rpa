...
int main(int argc, char *argv[]) {
    ...
    const char *pattern = "([0-9]{1,3})"; // Regex pattern to match numbers
    const char *title = "SRE Example 5 Result"; // Title for the message box
    regex_t regex;
    ...
    if (regexec(&regex, input, 2, matches, 0) == 0) {
        char result[100];
        snprintf(result, sizeof(result), "%.*s", (int)(matches[0].rm_eo - matches[0].rm_so), input + matches[0].rm_so);
        
        show_message(title, result); // Pass the title and result to the show_message function
    } else {
